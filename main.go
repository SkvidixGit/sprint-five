...
func (w Walking) Calories() float64 {
	// вставьте ваш код ниже
	return ((CaloriesWeightMultiplier*w.Weight + (math.Pow(w.meanSpeed()*KmHInMsec/(w.Height/CmInM), 2))*CaloriesSpeedHeightMultiplier*w.Weight) * float64(w.Duration.Hours()) * MinInHours)
}
...
type Swimming struct {
	// добавьте необходимые поля в структуру
	Training
	LengthPool int64 // длина бассейна
	CountPool  int64 // количество пересечений бассейна
}
